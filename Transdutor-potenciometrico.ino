...
void loop() {    
  ...
  //Serial.println(voltagem); //exibe no monitor serial o valor da voltagem;
   //Limpa a tela
  //lcd.clear();
  lcd.setCursor(0, 0); //coloca um espaçamento e seleciona primeira linha
  lcd.print("Vout="); //imprimi mensagem no lcd
  lcd.setCursor(5, 0); //coloca um espaçamento e seleciona segunda linha
  lcd.print(voltagem); //imprimi o valor lido pela porta analógica(A1) e exibe no lsd
  lcd.print("V");
  lcd.setCursor(0,1);
  lcd.print("Vbits=");
  lcd.print(valorPortaAnalogica);
  }
